...
def replace_leaf(t, find_value, replace_value):
    ...
    """
    "*** YOUR CODE HERE ***"
    return tree(replace_value if is_leaf(t) and label(t) == find_value else label(t), [replace_leaf(b, find_value, replace_value) for b in branches(t)])
...
def has_path(t, word):
    ...
    "*** YOUR CODE HERE ***"
    return label(t) == word[0] and (all( [ has_path(b, word[1:])] for b in branches(t)) if len(word) != 1 else True)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) and lower_bound(y)
...
def check_par():
    ...
    True
    """
    r1 = interval(1, 10000) # Replace this line!
    r2 = interval(1, 10000) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    if x1 <= s <= x2:
        if a > 0:
            return interval(fun(s), max(fun(x1), fun(x2)))
        else:
            return interval(min(fun(x1), fun(x2)), fun(s))
    else:
        return interval(min(fun(x1), fun(x2)), max(fun(x1), fun(x2)))
